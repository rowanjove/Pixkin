Create a single horizontal sprite strip for the Codex app digital pet `linlin` in the state `jumping`.
...
Output exactly 5 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Preserve black-purple high ponytail, light-purple star accessory, cool eyes, white technical outfit, restrained reliable expression. Simplify strongly into a compact mascot..
