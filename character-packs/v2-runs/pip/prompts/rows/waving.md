Create a single horizontal sprite strip for the Codex app digital pet `pip` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Preserve Pip as the recognizable purple Pixkin mascot: rounded compact body, cat-like ears, expressive face, purple palette, friendly mischievous energy..
