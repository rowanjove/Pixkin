Create a single horizontal sprite strip for the Codex app digital pet `shanshan` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Preserve silver-white long hair, blue eyes, calm warm expression, and the small book as the signature prop. Gentle, intelligent, quiet reading companion..
