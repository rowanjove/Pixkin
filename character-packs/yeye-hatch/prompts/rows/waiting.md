Create a single horizontal sprite strip for the Codex app digital pet `yeye` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: 角色显示名为椰子。保留浅金白发、左右双卷角、金色眼睛、大号金色蝴蝶结、奶油白与深棕金服装；将人形设计转化为紧凑Q版数字桌宠。.
