...
- 459 项测试与 46 个子测试通过，总覆盖率 74.99%。
- Ruff、Pyright（Python 3.11）通过；本轮修复后的文件夹版与便携版已重建并通过隔离烟测，
  依赖漏洞审计、CycloneDX SBOM 和安装器尚未重新执行。
- 更新协议覆盖签名篡改、字段注入、渠道错配、降级、最低兼容版本、HTTP
  降级、大小、哈希、不完整下载和用户确认测试。
- 本机私钥签名安装器清单，再由成品公钥复验的端到端链路通过。
- Inno Setup 6.7.3 编译通过。
- 安装器、发布目录、SBOM、统一哈希和正式更新清单仍需在本轮源码修复后重新生成，当前不视为已复验。
